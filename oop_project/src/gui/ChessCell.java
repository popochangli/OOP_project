...
public class ChessCell extends Pane {
    ...
    public void draw(Piece piece) {
        ...
        if (piece != null && piece.getImage() != null) {
            ...
            BackgroundImage bgImg = new BackgroundImage(piece.getImage(), BackgroundRepeat.NO_REPEAT, BackgroundRepeat.NO_REPEAT, BackgroundPosition.CENTER, new BackgroundSize(cellWidth, cellHeight, false, false, false, false));
            this.setBackground(new Background(new BackgroundFill[]{new BackgroundFill(cellColor, CornerRadii.EMPTY, Insets.EMPTY)}, new BackgroundImage[]{bgImg}));
    ...
    private void initializeDragAndDrop() {
        ...
        this.setOnDragDetected(event -> {
            ...
            System.out.println("Drag detected: Piece " + piece.getClass().getSimpleName() + " (" + piece.getTeam() + ") is being moved from position (" + GridPane.getColumnIndex(this) + ", " + GridPane.getRowIndex(this) + ").");
            ...
            System.out.println("Dragboard content set: {image=" + piece.getImage() + ", type=" + piece.getClass().getSimpleName() + piece.getTeam() + "}");
        ...
        this.setOnDragDropped(event -> {
            ...
            if (db.hasImage() && db.hasString()) {
                ...
                if (movingPiece != null) {
                    System.out.println("Drop detected: Piece " + movingPiece.getClass().getSimpleName() + " (" + movingPiece.getTeam() + ") is being dropped on position (" + targetX + ", " + targetY + ").");
            ...
            for (int y = 0; y < chessPane.getChessPaneHeight(); y++) {
                ...
                if (targetCell != null && piece != null && piece.validateMove(startX, startY, x, y, chessPane)) {
                    ...
                    if (targetPiece == null || !"W".equals(targetPiece.getTeam())) {
                        targetCell.setBorder(new Border(new BorderStroke(Color.GREEN, BorderStrokeStyle.SOLID, CornerRadii.EMPTY, BorderWidths.DEFAULT)));
                    }
                }
            }
        }
    ...
    private boolean processMove(Piece movingPiece) {
        ...
        if (hasPiece()) {
            ...
            if (!piece.getTeam().equals(movingPiece.getTeam())) {
                ...
                System.out.println("Transforming piece: " + piece.getClass().getSimpleName() + " (" + piece.getTeam() + ") to team " + movingPiece.getTeam());
